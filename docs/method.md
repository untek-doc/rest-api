...
Отличие между PUT и PATCH заключается в том, что PUT изменияет сразу все атрибуты, 
а PATCH изменяет атрибуты частично.
Т.е. если нужно изменить только заголовок статьи, то отправляем только заголовок с методом PATCH.
